...
class NoTankAiV15D20241230(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        # pair = metadata["pair"]
        if 'BTC' in metadata["pair"]:
            self.dp.send_msg(
                f'Begin {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {metadata["pair"]} {dataframe.iloc[-1, :6].values}')
        ...
        if 'BTC' in metadata["pair"]:
            self.dp.send_msg(
                f'{datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {metadata["pair"]} {dataframe.iloc[-1, :6].values}')
        # if dataframe["maxima"].iloc[-3] == 1 and dataframe["maxima_check"].iloc[-1] == 0:
        #     self.dp.send_msg(f'{metadata["pair"]} populate indicators,Maxima Detected - Potential Short!!!')
        # if dataframe["minima"].iloc[-3] == 1 and dataframe["minima_check"].iloc[-1] == 0:
        #     self.dp.send_msg(f'{metadata["pair"]} populate indicators,Maxima Detected - Potential Long!!!')
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        if 'BTC' in metadata["pair"]:
            self.dp.send_msg(
                f'entry {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {metadata["pair"]} {df.iloc[-1, :6].values}')
        return df
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        if 'BTC' in metadata["pair"]:
            self.dp.send_msg(
                f'exit {datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {metadata["pair"]} {df.iloc[-1, :6].values}')
        return df
